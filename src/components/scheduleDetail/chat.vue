<template>
  <div class="chat">
    <div class="chat__content">
      <div class="message" v-for="(item, index) in setMsgBox" :key="index">
        <li v-if="checkStyle(item)">
          <div v-if="item.chatMessage.style === 'date'" class="date">{{item.chatMessage.time}}</div>
          <div v-else class="welcome"><p>{{item.chatMessage.from.userUuid}}</p> {{item.chatMessage.message}}</div>
        </li>
        <li v-else :class="{'self': item.chatMessage.from.userUuid === userInfo.userUuid}">
...
import { setTimeBar } from '@/lib/setTimeBar'
...
export default {
  ...
  methods: {
    ...
    */
    ...
    /** @summary 判斷 style & msg
    * @param {Object} item 判斷的資料
    * @return
    */
    checkStyle (item) {
      if (item.chatMessage.style === 'date' || (item.chatMessage.style === 'sys' && item.chatMessage.message === 'enter room')) {
        return true
      }
      return false
    }
